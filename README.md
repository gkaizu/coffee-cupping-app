...
Remix (TypeScript): Web標準を重視した設計思想と、高いエッジ・パフォーマンスを評価し採用。
...
graph TD
    ...
    CloudFront --> AppRunner[App Runner / Remix]
...
Web標準を最大限に活用し、サーバーサイドでのデータ処理を効率化できる点を重視しました。また、モダンなEC開発（Hydrogen等）との親和性が高く、将来的なエコシステムへの適応も視野に入れています。